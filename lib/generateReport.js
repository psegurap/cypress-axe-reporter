...
const generateReport = async (config, files) => {
    let customReporterOptions = {
        reportFilename: "cypress-axe-reporter",
        reportDirJson: defaultJSONA11yOutputFolder,
        reportDirHTML: defaultHTMLOutputFolder,
        reportTitle: "cypress-axe-reporter",
        reportPageTitle: "cypress-axe-reporter",
    };

    if (config.reporterOptions) {
        if (config.reporterOptions.reportDir) {
            customReporterOptions.reportDirJson =
                config.reporterOptions.reportDir + "/json/";
            customReporterOptions.reportDirHTML =
                config.reporterOptions.reportDir + "/html/";
        }

        if (config.reporterOptions.reportFilename) {
            customReporterOptions.reportFilename =
                config.reporterOptions.reportFilename;
        }

        if (config.reporterOptions.reportPageTitle) {
            customReporterOptions.reportPageTitle =
                config.reporterOptions.reportPageTitle;
        }

        if (config.reporterOptions.reportTitle) {
            customReporterOptions.reportTitle =
                config.reporterOptions.reportTitle;
        }
    }

    fse.ensureDirSync(path.join("./", customReporterOptions.reportDirHTML));

    let html_structure = await generateHTMLStructure(files, customReporterOptions);
    console.log(customReporterOptions);
    let store_report_on_location = path.join(
        "./",
        customReporterOptions.reportDirHTML,
        customReporterOptions.reportFilename + ".html"
    );

    await fsePromises.writeFile(store_report_on_location, html_structure);

    console.log("REPORT SAVED ON: " + store_report_on_location);
};

const generateHTMLStructure = async (files, customReporterOptions) => {
    const dom = new JSDOM(
    ...
    if (styles_path_exist) {
        ...
        let title_head_tag = customCreateElement({
            tag: "title",
            text: customReporterOptions.reportPageTitle,
        });
    ...
    const createNavBar = () => {
        ...
        let h5_page_title = customCreateElement({
            ...
            text: customReporterOptions.reportTitle,
        });
    ...
    files.map((file) => {
        let test_result = fse.readJsonSync(
            path.join("./", customReporterOptions.reportDirJson, file)
        );
