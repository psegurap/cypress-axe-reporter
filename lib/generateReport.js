...
const generateHTMLReport = async (test_results) => {
    ...
    const dom = new JSDOM(
        `<html lang="en" data-theme="light"><head><head><body filter_by_impact="all"></body><html>`
    );
    ...
    const createNavBar = () => {
        ...
        let h1_page_title = customCreateElement({
            tag: "h1",
            classes: ["subtitle", "is-5", "has-text-white"],
            text: drOpts.reportTitle,
        });

        div_start_navbar_item.append(h1_page_title);
    ...
    const createFooter = () => {
        let footer_container = customCreateElement({
            tag: "footer",
            classes: ["footer", "is-size-7", "has-text-grey-dark"],
        });
    ...
    const generateSingleSpecCard = (spec) => {
        ...
        spec.a11y.violations.forEach((violation) => {
            ...
            let p_violation_helpURl_label = customCreateElement({
                tag: "p",
                classes: ["has-text-weight-medium", "is-size-7"],
                text: "Help: ",
            });

            let a_violation_helpURl_details =
                customCreateElement({
                    tag: "a",
                    classes: ["has-text-weight-normal", "is-underlined", "has-text-dark"],
                    text: violation.helpUrl,
                    attrs: [{key: "href", value: violation.helpUrl},{key: "target", value: "_blank"}]
                });

            p_violation_helpURl_label.append(a_violation_helpURl_details)
